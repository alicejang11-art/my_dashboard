...
st.info(
    ...
    "- 반면 중국은 한·중 관계 변화와 맞물려 특정 시점 이후 뚜렷한 감소가──────────────────────────────────────
with st.sidebar:
    st.header("🔧 필터")
    year_df = run_query("SELECT MIN(연도) AS min_y, MAX(연도) AS max_y FROM 해외취업연도별통계")
    min_year = int(year_df["min_y"].iloc[0])
...
st.info(
    ...
    "- 베트남과 중국의 구인처 수는 2020년 이후 빠르게 증가했으며, 특히 베트남은 2023년에 가장 높은 증가 폭을 보입니다.\n"
...
st.info(
    "💡 **인사이트**\n"
    "- 전반적으로 현지 청년실업률이 낮은(고용 안정적인) 국가일수록 한국인 취업자 수도 많은 음의 상관관계가 나타나는 것처럼 보입니다.\n"
    "- 단, 베트남처럼 실업률이 낮아도 임금 수준 차이로 취업자가 적은 '이상치' 국가가 존재합니다.\n"
    "- 그러나 실업률과 취업자 수 간 뚜렷한 선형 관계가 없어, 청년실업률 단독으로는 한국인의 해외 취업률과의 상관관계를 설명하기 어렵고 임금 수준·비자 정책 등 보조 변수가 필요합니다."
)
